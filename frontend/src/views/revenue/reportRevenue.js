...
import {viewRevenueOfACustomer} from '../../controllers/revenue';
...
export default function ReportPayment() {
    ...
    async function appointmentReportGet() {
        ...
        if (room === '') {
            ...
            setLoading(true);
            viewRevenueOfACustomer(roomId).then((result) => {
                console.log(result);
    ...
    function downloadPDF() {
        ...
        doc.autoTable({
            theme: "grid",
            head: [['Type', 'Details', 'Date', 'Time', 'Price']],
            body: reportList.map(col => [[col.type], [col.details], [col.date], [col.time], [col.price]]),
            margin: { top: 71 }
        })
        doc.save('RevenueReport' + room +'(' + new Date().toISOString().substring(0, 10) + " " + new Date().toLocaleTimeString('en-US') + ').pdf')
    }
                                            ...
                                            <tbody>
                                                ...
                                                {reportList.map((value, index) => {
                                                    return <tr key={index}  id="OrderId">
                                                        ...
                                                        <td class="text-start">{value.details}</td>
                                                        <td class="text-start">{value.date}</td>
                                                        <td class="text-start">{value.time}</td>
                                                        <td class="text-start">{"Rs. " + value.price}</td>
                                                    </tr>
                                                })}
                                            </tbody>
