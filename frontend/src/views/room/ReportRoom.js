...
export default function ReportRoom() {
    ...
    const getRoomDetails = async(room) => {
        await getSelectedRoomByCode(room).then((res) => {
            setRoomDetails(res[0]);
            console.log(res);
    ...
    async function appointmentReportGet() {
        ...
        if (room === '') {
            ...
            setLoading(true);
            getSelectedReservationByRoom(room).then((result) => {
                console.log(result);
    ...
    function downloadPDF() {
        ...
        doc.setFontSize('10')
        doc.setFont('Helvertica', 'bold')
        doc.text("Room price ", 133, 55)
        doc.setFontSize('10')
        doc.setFont('Helvertica', 'Normal')
        doc.text(":  Rs. " + roomDetails.price + ".00", 151, 55)
        ...
        doc.autoTable({
            theme: "grid",
            head: [['Customer Name', 'NIC', 'Phone Number', 'Checkin Date', 'Checkout Date', 'Total Persons']],
            body: reportList.map(col => [[col.name], [col.nic], [col.phoneNumber], [col.checkInDate.slice(0,10)],
                [col.checkOutDate.slice(0,10)], [Number(col.numOfAdults) + Number(col.numOfChildren)]]),
            margin: { top: 71 }
        })
        doc.save('ReservationReportForRoom' + room +'(' + new Date().toISOString().substring(0, 10) + " " + new Date().toLocaleTimeString('en-US') + ').pdf')
    }
                            ...
                            <div class="card">
                                <div class="card-body mt-3 mb-2" style={{ margin: "0px" }} >
                                    <div class="row mb-2 px-4">
                                        <h5 class="fw-semibold">Select a room to generate the report</h5>
                                    </div>
                                    ...
                                    <div class="row  align-items-center px-4 mb-2" >
                                        <div class="mb-3 col-md-6 mb-2 ml-2">
                                            ...
                                            <Select
                                                ...
                                                isSearchable
                                                options={roomList}
                                                onChange={((e) => {setRoom(e.label); setRoomId(e.value); getRoomDetails(e.label)})}
                                            />
                                        </div>
                                        <div class="col-md-2 " >
